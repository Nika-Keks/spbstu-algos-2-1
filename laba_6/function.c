...
char** arr_word(char** mas)
{
	...
	if (arr) {
		...
		for (int i = 0; i < count; i++)
		{
			if (repeat(mas[i], arr) == -1) // we check whether the word was written to the array
			{
...
{
	char** word_arr =arr_word(full_arr(filename));	
	char** str= string_arr(filename);
	tree* tree_n=create_Node(word_arr[0], str);//creating the root of the tree
	for (int i = 1; i < count_word; i++)
	{
		insert(&tree_n, word_arr[i], str);//inserting elements
	}
	free(str);
	free(word_arr);
	return tree_n;
